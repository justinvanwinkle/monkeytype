...
def shrink_typed_dict_types(
    ...
    """
    Shrink a list of TypedDicts into one with the required fields and the
      optional fields.
    Required fields are keys that appear as a required field in all the
      TypedDicts.
    ...
    If the same key has multiple value types, then its value is the Union of
      the value types.
    """
...
class RewriteGenerator(TypeRewriter):
    """
    Returns an Iterator, if the send_type and return_type of a
      Generator is None
    """
...
class RewriteMostSpecificCommonBase(TypeRewriter):
    ...
    def _merge_common_bases(self, first_bases, second_bases):
        ...
        for first_base, second_base in zip(
            first_bases, second_bases, strict=False
        ):
